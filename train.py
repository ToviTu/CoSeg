...
for batch in tqdm.tqdm(data_loader):
    ...
    output_masks, output_labels = model(pixel_values, source_ids, token_summary_idx=token_summary_idx)

    loss = loss_2(m(output_masks), target_masks[:, :output_masks.shape[1], :, :]) + alpha * loss_1(output_labels, target_labels)
    loss.backward()
    optim.step()
    optim.zero_grad()
